...
func loadMetricsAddr(lookup lookupEnvFunc, httpAddr string) (string, error) {
	metricsAddr, err := nonEmptyValue(lookup, "METRICS_ADDR", defaultMetricsAddr)
	if err != nil {
		return "", err
	}
	metricsEndpoint, err := listenEndpoint("METRICS_ADDR", metricsAddr)
	if err != nil {
		return "", err
	}
	...
	if metricsEndpoint == httpEndpoint {
		return "", fmt.Errorf("METRICS_ADDR must differ from HTTP_ADDR")
	}
