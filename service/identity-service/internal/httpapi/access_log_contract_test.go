...
func TestAccessLogContractIncludesRequiredFields(t *testing.T) {
	...
	if record["service"] != "identity-service" || record["request_id"] != "access-contract-request" || record["method"] != http.MethodGet || record["route"] != "/contract/{id}" {
...
func TestHealthAccessCompletionUsesDebugLevel(t *testing.T) {
	...
	debugLogger := slog.New(slog.NewJSONHandler(&debugLogs, &slog.HandlerOptions{Level: slog.LevelDebug})).With("service", "identity-service")
	...
	infoLogger := slog.New(slog.NewJSONHandler(&infoLogs, nil)).With("service", "identity-service")
