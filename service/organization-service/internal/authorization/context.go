...
func NewActorContext(identityUserID, organizationID, membershipID uuid.UUID, role string, permissions []string) ActorContext {
	...
	for _, permission := range permissions { permissionSet[permission] = struct{}{} }
...
func (a ActorContext) HasPermission(permission string) bool { _, ok := a.permissions[permission]; return ok }
func (a ActorContext) Permissions() []string {
	permissions := make([]string, 0, len(a.permissions))
	for permission := range a.permissions { permissions = append(permissions, permission) }
