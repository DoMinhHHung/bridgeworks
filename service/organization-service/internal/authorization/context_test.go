...
func TestActorContextCopiesAndSortsPermissions(t *testing.T) {
	...
	if !actor.HasPermission("organization.read") || actor.HasPermission("tampered") { t.Fatal("actor permissions were not copied") }
	want := []string{"membership.manage", "organization.read"}
	if got := actor.Permissions(); !reflect.DeepEqual(got, want) { t.Fatalf("Permissions()=%v want %v", got, want) }
}
