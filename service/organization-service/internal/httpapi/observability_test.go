...
func TestHTTPObservabilityCapturesBoundedCompletionRecords(t *testing.T) {
	...
	router.Get("/organizations/{id}", func(w http.ResponseWriter, _ *http.Request) { _, _ = w.Write([]byte("ok")) })
	router.Get("/no-content", func(w http.ResponseWriter, _ *http.Request) { w.WriteHeader(http.StatusNoContent) })
	...
	tests := []struct { path string; status int; route string }{
		...
		{path: "/panic/provider-secret", status: 500, route: "/panic/{id}"},
		{path: "/missing/provider-secret", status: 404, route: "unknown"},
	}
